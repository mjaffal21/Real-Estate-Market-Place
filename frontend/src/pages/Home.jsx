...
const Home = () => {
  const { data: offersListing } = useGetOfferListingsQuery();
  const { data: rentsListing } = useGetRentListingsQuery();
  const { data: salesListing } = useGetSaleListingsQuery();
  return (
    <div>
      {/* top */}
          ...
          offersListing.map((listing) => (
            <SwiperSlide key={listing._id}>
              <div
                style={{
                  background: `url(${
                    `${BASE_URL}/uploads` + listing.imageUrls[0]
                  }) center no-repeat`,
